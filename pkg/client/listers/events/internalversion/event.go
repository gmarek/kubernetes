...
type EventLister interface {
	...
	List(selector labels.Selector) (ret []*events.Event, err error)
...
// List lists all Events in the indexer.
func (s *eventLister) List(selector labels.Selector) (ret []*events.Event, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*events.Event))
	})
	return ret, err
}
...
type EventNamespaceLister interface {
	...
	Get(name string) (*events.Event, error)
...
// List lists all Events in the indexer for a given namespace.
func (s eventNamespaceLister) List(selector labels.Selector) (ret []*events.Event, err error) {
...
// Get retrieves the Event from the indexer for a given namespace and name.
func (s eventNamespaceLister) Get(name string) (*events.Event, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(events.Resource("event"), name)
	}
	return obj.(*events.Event), nil
}
