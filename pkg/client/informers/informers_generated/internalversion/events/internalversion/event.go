...
import (
	...
	internalversion "k8s.io/kubernetes/pkg/client/listers/events/internalversion"
	time "time"
)
	...
	sharedIndexInformer := cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				return client.Events().Events(v1.NamespaceAll).List(options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				return client.Events().Events(v1.NamespaceAll).Watch(options)
			},
		},
		&events.Event{},
		resyncPeriod,
...
func (f *eventInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&events.Event{}, newEventInformer)
}
